...
/// <summary>党建知识库问答服务（知识库优先 + 统一AI模型兜底）</summary>
public class AiKnowledgeService : IAiKnowledgeService
{
    ...
    public AiKnowledgeService(IAiModelService aiModelService)
    {
    ...
    public async Task<AiKnowledgeQueryResponse> QueryAsync(AiKnowledgeQueryRequest request)
    {
        ...
        // 第一优先级：知识库关键词匹配
        bool foundInKnowledgeBase = false;
        ...
        {
            answer = await _aiModelService.QueryKnowledgeAsync(question);
            confidence = 0.7;
            references.Add("AI模型生成");
        }
