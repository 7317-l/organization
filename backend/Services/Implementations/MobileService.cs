...
public class MobileService : IMobileService
{
    ...
    public MobileService(AppDbContext context, IMapper mapper, IPointService pointService)
    {
        _context = context;
        _mapper = mapper;
    ...
    public async Task<ReportProgressResponse> ReportProgressAsync(int memberId, ReportProgressRequest request)
    {
        ...
        bool isFirstLearn = progress == null;
        bool wasCompleted = progress?.IsCompleted ?? false;
        ...
        // ===== 学习得分逻辑：观看/阅读学习中心内容自动加分 =====
        int earnedPoints = 0;
        // 1. 首次学习内容（之前没有进度记录）：加10分
        if (isFirstLearn)
        {
            await _pointService.AddPointsAsync(memberId, 10, PointSourceType.WatchVideo, request.ContentId);
            earnedPoints += 10;
        }
        // 2. 首次完成内容（isCompleted=true 且之前未完成）：额外加10分
        ...
        // 计算进度百分比（已完成=100，有学习记录但未完成=50，无记录=0）
        int progressPercent = progress.IsCompleted ? 100 : (progress.DurationSeconds > 0 ? 50 : 0);
        ...
        return new ReportProgressResponse
        {
            ...
            TotalPoints = member?.PointTotal ?? 0
        };
    }
    ...
    public async Task<SubmitExamResponse> SubmitExamAsync(int memberId, SubmitExamRequest request)
    {
        ...
        // ===== 考试得分逻辑：完成测验后根据得分加积分 =====
        // 积分 = 得分 / 10（向下取整），最少5分，最多20分
        int examPoints = Math.Max(5, Math.Min(20, earnedScore / 10));
        await _pointService.AddPointsAsync(memberId, examPoints, PointSourceType.CompleteExam, request.TestId);

        // ===== 任务关联试卷完成后自动关闭任务 =====
        var currentMember = await _context.PartyMembers.FindAsync(memberId);
        var relatedTasks = await _context.LearningTasks
            .Where(t => t.TestId == request.TestId && t.Status == 0 && t.TargetOrgId == currentMember.OrganizationId)
            .ToListAsync();
        foreach (var task in relatedTasks)
        {
            ...
            var completedContents = await _context.MemberLearningProgress
                ...
                .ToListAsync();
            bool allContentsCompleted = taskContents.All(cid => completedContents.Contains(cid));
