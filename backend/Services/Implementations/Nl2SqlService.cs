...
/// <summary>NL2SQL服务（统一AI模型优先 + 模拟兜底，含安全过滤和错别字修正流程）</summary>
public class Nl2SqlService : INl2SqlService
{
    ...
    public Nl2SqlService(IAiModelService aiModelService)
    {
    ...
    public async Task<Nl2SqlResponse> QueryAsync(Nl2SqlRequest request)
    {
        ...
        // 步骤3：生成SQL（优先统一AI模型，失败则使用模拟）
        string sql;
        bool usedAiModel = false;
        try
        {
            sql = await _aiModelService.Nl2SqlAsync(nl);
            usedAiModel = !string.IsNullOrWhiteSpace(sql);
        }
        catch
        {
            sql = string.Empty;
        ...
        if (!usedAiModel || string.IsNullOrWhiteSpace(sql))
        {
            sql = GenerateMockSql(intent, nl);
        }
        ...
        {
            GeneratedSql = sql,
            Explanation = $"已识别意图：{intent}。{(usedAiModel ? "SQL由统一AI模型生成。" : "使用模拟SQL（AI模型未配置）。")}查询结果如下。",
            ResultData = mockData,
            ChartData = chartData,
            SessionId = request.SessionId ?? Guid.NewGuid().ToString("N"),
            CorrectionsApplied = corrections
