...
public class PartyDevelopmentService : IPartyDevelopmentService
{
    ...
    public async Task UpdateAsync(int id, UpdatePartyDevelopmentRequest request)
    {
        var p = await _context.PartyDevelopmentProcesses.FindAsync(id);
        if (p == null) throw new BusinessException("记录不存在", 404);

        if (request.Materials != null)
            p.MaterialsJson = JsonMappingHelper.ToJson(request.Materials);
        if (request.ReportContent != null)
            p.ReportContent = request.ReportContent;
