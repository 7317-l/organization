...
public class MobileController : ControllerBase
{
    ...
    /// <summary>上报学习进度（返回最新进度和积分信息，支持前端即时更新）</summary>
    [HttpPost("progress")]
    public async Task<ApiResponse> ReportProgress([FromBody] ReportProgressRequest request)
    {
        var result = await _service.ReportProgressAsync(_currentUser.UserId, request);
        return ApiResponse.Success(result, "进度已上报");
    }
