...
// 巩固练习模式：题目来自 sessionStorage，提交时本地判分（不写考试记录）
const isConsolidation = ref(false)
...
async function handleSubmit() {
  ...
  try {
    ...
    // ===== 巩固练习：本地判分（与后端 MobileService 归一化规则一致），不写考试记录 =====
    if (isConsolidation.value) {
      // 答案归一化：字母(A) / 数字索引(0) / 选项文本 / 判断(true/对) 统一转成选项文本
      const normalizeAns = (q, raw) => {
        const opts = q.options || []
        const t = String(raw ?? '').trim()
        if (!t) return ''
        if (opts.some(o => o === t)) return t
        ...
        if (/^\d+$/.test(t)) {
          ...
          if (num >= 0 && num < opts.length) return opts[num]
        }
        if (/^(true|对|正确)$/i.test(t)) return '正确'
        if (/^(false|错|错误)$/i.test(t)) return '错误'
        return t
      }
      // 多选答案集合归一化
      const normalizeMulti = (q, rawArr) => {
        const set = new Set()
        for (const t of rawArr) {
          const n = normalizeAns(q, t)
          if (n) set.add(n)
        }
        return set
      }
      const convertedQuestions = questions.value.map((q, idx) => {
        const ans = answers.value[idx]
        let userAnswerStr = ''
        let isCorrect = false
        if (q.type === 'judge') {
          userAnswerStr = (ans === true || ans === 'true' || ans === '对') ? '正确' : (ans === false || ans === 'false' || ans === '错') ? '错误' : ''
          isCorrect = userAnswerStr !== '' && userAnswerStr === normalizeAns(q, q.correctAnswer)
        } else if (q.type === 'multiple') {
          const userArr = Array.isArray(ans) ? ans : []
          userAnswerStr = userArr.length > 0 ? userArr.map(i => normalizeAns(q, i)).join('、') : ''
          // 正确答案兼容 "A,C" 或 JSON 数组
          let correctTokens = []
          const cRaw = String(q.correctAnswer ?? '').trim()
          if (cRaw.startsWith('[')) {
            try { correctTokens = JSON.parse(cRaw) } catch { correctTokens = cRaw.split(/[,，;； ]/) }
          } else {
            correctTokens = cRaw.split(/[,，;； ]/).filter(Boolean)
          }
          const uSet = normalizeMulti(q, userArr)
          const cSet = normalizeMulti(q, correctTokens)
          isCorrect = userArr.length > 0 && uSet.size === cSet.size && [...uSet].every(x => cSet.has(x))
        } else {
          userAnswerStr = typeof ans === 'number' ? String.fromCharCode(65 + ans) : (ans !== null && ans !== undefined ? String(ans) : '')
          const userNorm = normalizeAns(q, ans)
          isCorrect = ans !== null && ans !== undefined && userNorm !== '' && userNorm === normalizeAns(q, q.correctAnswer)
        }
        return {
          id: q.id || idx,
          question: q.question,
          options: q.options || [],
          type: q.type,
          userAnswer: formatAnswerForDisplay(userAnswerStr, q.type),
          correctAnswer: formatAnswerForDisplay(q.correctAnswer, q.type),
          isCorrect,
          score: q.score || 5,
      ...
      const totalScore = convertedQuestions.reduce((s, q) => s + (q.isCorrect ? q.score : 0), 0)
      examResult.value = {
        title: quizData.value?.title || '专项巩固练习',
        score: totalScore,
        questions: convertedQuestions,
        correctCount,
        wrongCount,
        correctRate: convertedQuestions.length > 0 ? Math.round((correctCount / convertedQuestions.length) * 100) : 0,
        durationUsed: timeUsed.value || 0
      }
      submitted.value = true

      // 答题记录同步到 AI 面板（错题分析联动）
      ...
      aiDataStore.setCurrentQuestion(null)
      sessionStorage.removeItem('consolidation_practice')
      ElMessage.success('巩固练习完成')
      return
    }
...
async function loadQuiz() {
  ...
  try {
    ...
    // 巩固练习：优先读取「AI专项巩固练习」传入的题目（sessionStorage）
    let cons = null
    try { cons = JSON.parse(sessionStorage.getItem('consolidation_practice') || 'null') } catch { cons = null }
    if (cons && cons.questions && cons.questions.length > 0) {
      isConsolidation.value = true
      const typeMap = { 0: 'single', 1: 'multiple', 2: 'judge' }
      quizData.value = { title: cons.title || '专项巩固练习', name: cons.title || '专项巩固练习' }
      questions.value = cons.questions.map(q => ({
        id: q.id || q.questionId,
        question: q.stem || q.question || q.title || '',
        type: typeMap[q.questionType] || q.type || 'single',
        options: Array.isArray(q.options) ? q.options : [],
        correctAnswer: q.correctAnswer || q.answer || '',
        knowledgePoint: q.categoryName || q.knowledgePoint || q.category || '',
        score: q.score || 5
      }))
      if (questions.value.length === 0) {
        ElMessage.warning('该练习卷暂无题目')
      }
      // 初始化答案
