<template>
  <div>
    ...
    <div class="aip-form">
      <div class="aip-field">
        <span class="aip-field-label">学习目标</span>
        <el-input v-model="target" placeholder="例如：系统掌握党史理论，提升测验成绩" style="width: 260px" />
      </div>
      <div class="aip-field">
    ...
    <div v-if="result" class="aip-result">
      <div class="aip-toolbar">
        <div class="aip-result-title">{{ result.memberName || '我' }} · {{ result.currentLevel || '当前水平' }} → {{ result.target || target || '学习目标' }}</div>
      </div>
      <div class="aip-meta">
